...
function setup() {
  ...
  for (let i = 2; i< 6; i++){
    ...
    const x = width / 2;
    const y = height / 2;
    centerBlobs.push(new Blobs(offsets, SPEED, x , y, R / (3*i)  ))
  }
}
...
function draw() {
  ...
  const excitementMultiplyer1 = map(excitement, 0.9, 3.0, 1, 200)
  ...
  for (let i= 0; i< centerBlobs.length; i++){
    let blob = centerBlobs[i];
    blob.x += sin(centerT  + blob.offsets[0]) * 2;
    blob.y += cos(centerT + blob.offsets[0]) * 2;
    if(i == 0){
      blendMode(BLEND);
      fill(200,30,35,90)
    }
